...
export function initPlayerEngine(audioRef: React.MutableRefObject<HTMLAudioElement | null>) {
  ...
  const seekTo = (time: number) => {
    if (audioRef.current) {
      ...
      const preSeekTime = isNative && (audioRef.current as any).nativeCurrentTime !== undefined
        ? (audioRef.current as any).nativeCurrentTime
        : audioRef.current.currentTime;
      audioRef.current.currentTime = time;
      if (isNative) {
        (audioRef.current as any).nativeCurrentTime = time;
        (audioRef.current as any).lastSeekTimestamp = Date.now();
        (audioRef.current as any).lastSeekTarget = time;
        (audioRef.current as any).preSeekTime = preSeekTime;
        QobuzAudio.seek({ time });
      } else {
        delete (audioRef.current as any).nativeCurrentTime;
      }
      (audioRef.current as any).isManualSeek = true;
      (audioRef.current as any).requestLyricsRedraw?.();
    }
  };
